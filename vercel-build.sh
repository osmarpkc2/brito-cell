...
pip install -r requirements.txt || error "Falha ao instalar as dependências do Python"
...
if [ -n "$POSTGRES_URL" ] || [ -n "$DATABASE_URL" ]; then
    ...
    pip install psycopg2-binary || warning "Falha ao instalar psycopg2, tentando continuando sem ele..."
else
    warning "Usando SQLite como banco de dados..."
    export DATABASE_URL="sqlite:///britocell.db"
fi
